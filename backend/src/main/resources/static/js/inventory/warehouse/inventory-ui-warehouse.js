    ...
    paginatedInventories.forEach((inventory) => {
      ...
      inventoryTableHtml += `
        ...
        inventory.imgUrl ? "" : "no-image"
                    ...
                    <td class="py-3 px-4">
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="if(typeof window.viewInventory === 'function') { window.viewInventory('${inventory.id}'); } else if(typeof viewInventory === 'function') { viewInventory('${inventory.id}'); }" class="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors" title="Ver detalles">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button onclick="if(typeof window.showInventoryManagerAssignment === 'function') { window.showInventoryManagerAssignment('${inventory.id}'); } else if(typeof showInventoryManagerAssignment === 'function') { showInventoryManagerAssignment('${inventory.id}'); }" class="p-2 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg transition-colors" title="Asignar Rol">
                                <i class="fas fa-user-tie"></i>
                            </button>
                            <button onclick="if(typeof window.showInventoryTreeModal === 'function') { window.showInventoryTreeModal('${inventory.id}', '${(inventory.name || "").replace(/'/g, "\\'")}', '${(inventory.imgUrl || "").replace(/'/g, "\\'")}'); } else if(typeof showInventoryTreeModal === 'function') { showInventoryTreeModal('${inventory.id}', '${(inventory.name || "").replace(/'/g, "\\'")}', '${(inventory.imgUrl || "").replace(/'/g, "\\'")}'); }" class="p-2 text-[#00AF00] hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors" title="Ver jerarquía">
        ...
        inventory.imgUrl ? "" : "no-image"
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex gap-2">
                            <button onclick="if(typeof window.viewInventory === 'function') { window.viewInventory('${inventory.id}'); } else if(typeof viewInventory === 'function') { viewInventory('${inventory.id}'); }" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors" title="Ver detalles">
                                <i class="fas fa-eye text-lg"></i>
                            </button>
                            <button onclick="if(typeof window.showInventoryManagerAssignment === 'function') { window.showInventoryManagerAssignment('${inventory.id}'); } else if(typeof showInventoryManagerAssignment === 'function') { showInventoryManagerAssignment('${inventory.id}'); }" class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 transition-colors" title="Asignar Rol">
                                <i class="fas fa-user-tie text-lg"></i>
                           </button>
                           <button onclick="if(typeof window.showInventoryTreeModal === 'function') { window.showInventoryTreeModal('${inventory.id}', '${(inventory.name || "").replace(/'/g, "\\'")}', '${(inventory.imgUrl || "").replace(/'/g, "\\'")}'); } else if(typeof showInventoryTreeModal === 'function') { showInventoryTreeModal('${inventory.id}', '${(inventory.name || "").replace(/'/g, "\\'")}', '${(inventory.imgUrl || "").replace(/'/g, "\\'")}'); }" class="text-[#00AF00] hover:text-green-800 dark:hover:text-green-600 transition-colors" title="Ver jerarquía">
